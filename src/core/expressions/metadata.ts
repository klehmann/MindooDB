...
export const mindooDBViewLanguageHelpers = [
  ...
  helper("parent", "context", "Read a field of the parent document of a nested lookup.", "Inside an include filter of a nested query (MindooQuery.include), resolves a path against the PARENT row's summary fields instead of the document being evaluated (use the mirrored \"_lastModified\" for the timestamp). For the parent's id use parentDocId(). Only valid in an include filter: everywhere else there is no parent row and the query engine rejects it.", "parent(path)", "Expression<T>", [
    { name: "path", kind: "string", description: "A dot-separated field path of the parent document." },
  ], ["v.parent(\"customerId\")", "v.eq(v.field(\"status\"), v.parent(\"status\"))"]),
  helper("docId", "context", "Return the current document id.", "Reads the id of the document being evaluated. A document id is metadata rather than a stored field, so it is not reachable through field(\"docId\") — that reads a field of the same name, which normally does not exist.", "docId()", "Expression<string>", [], ["v.docId()"]),
  helper("parentDocId", "context", "Return the parent document id of a nested lookup.", "Inside an include filter of a nested query (MindooQuery.include), returns the id of the PARENT row — the docId() counterpart of parent(). Joining a child to its parent by id is its main use. Only valid in an include filter: everywhere else there is no parent row and the query engine rejects it.", "parentDocId()", "Expression<string>", [], ["v.eq(v.field(\"invoiceId\"), v.parentDocId())"]),
  ...
  helper("lastModifiedAt", "context", "Return the current document last-modified timestamp.", "Reads the ISO-8601 last-modified timestamp provided as document metadata by the host runtime (from `getLastModified()` / `_lastModified`).", "lastModifiedAt()", "Expression<string | null>", [], ["v.lastModifiedAt()"]),
