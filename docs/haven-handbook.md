...
In the result tree you can expand categories, drill into documents, and use the checkboxes to build a selection. Selecting a category implicitly selects every visible descendant document under it. Export .xlsx turns that selection into a real workbook: it keeps the matching category rows above the selected documents and includes both the source metadata and any computed view columns, so you can hand the file straight to a non-MindooDB tool.

## Database Browser and DAG Explorer
...
When the plain revision list is not enough to explain what happened, open the DAG Explorer. It is a graph rendering of every signed lifecycle entry ever applied to the document. Time runs left to right: each node is one signed entry, and edges show how entries followed each other. Branches appear when two people edited the document at the same time, and they meet again at a merge node when the next sync brought their work together.
...
Hovering a node shows who made the change, when, from which device, and which fields were touched. Clicking a non-snapshot node materializes the document as it stood on that branch and shows the result in the Materialized State panel, together with the attachments that existed at that point. Haven either reuses a nearby snapshot or replays the entry chain from the branch roots, and it tells you which of the two it did. A second panel, Conflict Analysis, lists the conflict paths attached to the entry you selected, because MindooDB's Automerge engine resolves concurrent edits by a deterministic rule rather than guessing.

Nothing in the DAG Explorer can be edited. It is a faithful, read-only record of history. That is also what makes it useful for compliance: every node is signed by the user who made the change, so it is the source of truth when a reviewer asks who changed this and when.
...
A big number at the top sums everything Haven currently keeps in this browser: every local database, every cache, every hosted app file. A Refresh button recalculates after big operations like a sync, a delete, or a cache clear. Numbers do not auto-refresh because measuring large stores can be slow. Next to it, Reclaim incomplete uploads frees the encrypted chunks left behind by attachment uploads that never finished; attachments that made it into document history are never touched.
...
Below each tenant's databases you will also see its caches: a local tenant cache for general per-tenant state, a server target cache for each MindooDB server the tenant talks to, a virtual view cache for each saved view, and a Full-text index (MiniSearch) entry for every database that has full-text indexing switched on. Each cache shows its own size, and virtual view caches can be cleared individually if one particular view has grown too large.
...
Because every change is signed and appended to a chain, history cannot be rewritten quietly. Haven's DAG Explorer is a direct rendering of that chain, and Automerge merges concurrent edits deterministically so two people can work on the same document without a conflict dialog.
...
DAG Explorer — a graph view of every change ever applied to a document, including how concurrent edits were merged together.
...
Haven Enterprise is a commercial edition Mindoo GmbH is building out, on exactly the same MindooDB core. Its features unlock with an Enterprise license key, and they arrive one at a time rather than all at once. Workspace roaming across devices and browser profiles is the first one you can use today: the Roamed workspace setting under Preferences → General is exactly this feature, which is why the toggle stays off until a license is imported. Still being built are custom branding (logo, colors, product name, and domain so Haven feels like your own product), a managed user interface with organisation-specific defaults, managed workspace templates pushed to users so they land in a configured environment, an in-house app store for curating internal and third-party MindooDB apps, automatic scheduled backups of in-browser data, first-class backup tooling for the tenant data stored on your server, and inline editing of common Office and text attachments without having to download them. The list continues to grow as the Enterprise client matures — customer feedback shapes the roadmap directly.
