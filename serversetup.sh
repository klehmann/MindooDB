...
# and writes a `.env` file for docker compose.
# ---------------------------------------------------------------------------
...
banner "Writing .env for docker compose"

cat > .env <<ENV_EOF
# Generated by serversetup.sh — used by docker-compose.yml.
MINDOODB_DATA_DIR=${DATA_DIR_ABS}
MINDOODB_BIND_ADDR=${BIND_ADDR}
MINDOODB_RUNTIME_UID=${HOST_UID}
MINDOODB_RUNTIME_GID=${HOST_GID}
MINDOODB_DATA_MOUNT_SUFFIX=${DATA_MOUNT#${SERVER_DATA_ABS}:/data}
MINDOODB_PASSWORD_MOUNT_SUFFIX=${PASSWORD_MOUNT#${PASSWORD_FILE_ABS}:/run/secrets/server_unlock:ro}
ENV_EOF

if [[ -f docker-compose.override.yml ]]; then
  rm -f docker-compose.override.yml
  info "Removed stale docker-compose.override.yml to avoid duplicate port bindings"
fi

info "Environment file written to .env"
