...
write_override_file() {
  ...
  fi
  {
    ...
    echo "      # Uncomment when a proxy, load balancer, or CDN fronts this server."
    echo "      # Rate limits and the /system/* IP allowlist key on the client address;"
    echo "      # without this every proxied user shares the proxy's address, so one busy"
    echo "      # client spends everyone's budget. The value is how many proxies are in"
    echo "      # front: 1 behind Cloudflare alone, 2 when nginx also fronts the origin."
    echo "      # Avoid \"true\" — it trusts the whole X-Forwarded-For chain, which a"
    echo "      # client can forge to get a fresh budget per request."
    echo "      # MINDOODB_TRUST_PROXY: \"1\""
    echo "    volumes:"
    echo "      - \"${DATA_MOUNT}\""
    echo "      - \"${PASSWORD_MOUNT}\""
    echo "    ports:"
