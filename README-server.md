...
When both servers have `iroh.enabled`, `url` may be `iroh:<ticket>` instead of an HTTPS origin. Admin APIs stay on HTTP; only the replicator session uses QUIC. See [docs/iroh.md](docs/iroh.md).

Only `name` and the two public keys are required. The rest describe the replication session and all have defaults:

| Field | Values | Default | Meaning |
| --- | --- | --- | --- |
| `url` | HTTPS origin or `iroh:<ticket>` | *(none)* | Where to reach the peer. **Without it the server is trusted but never dialled** — it can push to us, we never pull from it. |
...
Generated by `pnpm server:init`. Contains a `PrivateUserId` with encrypted private keys:
...
`url` is either an HTTPS origin (`https://server2.example.com`) or an Iroh locator (`iroh:<ticket>` / raw `endpoint…` ticket). Cluster admin stays on HTTP; only the replicator session uses Iroh. Store a full ticket to pair — `iroh:<64-hex>` is a stable Node-ID label but cannot be dialled. A frozen ticket can go stale after an IP or relay change; paste a fresh one from the peer's Iroh log. See [docs/iroh.md](docs/iroh.md).
...
`username` from previous versions is **no longer required or used for matching** — clients may
authenticate by signing key alone, so the server never needs the name. Any
`username` field is treated as documentation only; use `comment` (or any other
extra field) for human-readable notes. Entries are matched solely by
`signingPublicKey`.
...
### `pnpm server:init` — Initialize server identity
...
### `pnpm identity:info` — Show public information from an identity file
...
### `pnpm identity:change-password` — Re-encrypt an identity with a new password
...
### `pnpm identity:export-public` — Export only the public portion of an identity
...
```bash
MINDOODB_SERVER_PASSWORD_FILE=./.server-password pnpm server:dev -- --static-dir ./webapp-bootstrap
```
...
```bash
MINDOODB_SERVER_PASSWORD_FILE=./.server-password pnpm server:dev -- \
...
If `serversetup.sh` needs to regenerate the override later, it first creates a timestamped backup such as `docker-compose.override.20260407-103000.yml` and tells you where to find it so custom settings can be merged forward if needed.
...
```bash
MINDOODB_SERVER_PASSWORD_FILE=./.server-password pnpm server:dev -- -d ./data -p 1661
```

### Run built version

```bash
MINDOODB_SERVER_PASSWORD_FILE=./.server-password pnpm server:start -- -d ./data -p 1661
```
